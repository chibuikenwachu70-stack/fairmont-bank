...
def generate_transaction_reference():
    ...
    while True:
        ...
        if existing_transaction is None and existing_approval is None:
            return reference
...
def ensure_virtual_card_security_code_column():
    ...
    # Older installations already have a status column, but keep this
    # migration guard so the updated card controls work with an older
    # database without deleting any existing card records.
...
def dashboard():
    ...
    # Show only a currently usable card on the dashboard.
    # Cancelled cards are intentionally excluded so the dashboard
    # immediately returns to the "Get Virtual ATM Card" state.
    virtual_card = (
        VirtualATMCard.query
        .filter_by(customer_id=customer.customer_id)
        .filter(VirtualATMCard.status.in_(["Active", "Frozen"]))
        .first()
    )
...
def apply_virtual_card():
    ...
    # A cancelled card is no longer an active card. Remove its local
    # record so the customer can request a fresh virtual card using
    ...
    if existing_card is not None:
        flash(
            "You already have a virtual ATM card.",
            "success"
        )
...
def virtual_card_review():
    ...
    if card is None or card.status == "Cancelled":
        flash(
            "You do not have an active virtual ATM card yet.",
            "error"
        )
        return redirect(url_for("dashboard"))
...
# =========================================================
# VIRTUAL ATM CARD SECURITY CONTROLS
# =========================================================
# The virtual card is a local simulation. Card status is stored as
# Active, Frozen, or Cancelled. Security code is only exposed on
# the dedicated review page, never on the dashboard.


def _get_current_virtual_card():
    customer_id = session.get("customer_id")
...
def virtual_card_freeze():
    customer, card = _get_current_virtual_card()

    if customer is None:
        return redirect(url_for("login"))

    if card is None:
        flash("You do not have a virtual ATM card.", "error")
        return redirect(url_for("dashboard"))

    if card.status == "Cancelled":
        flash("This virtual ATM card has been cancelled and cannot be frozen.", "error")
        return redirect(url_for("virtual_card_review"))

    if card.status == "Frozen":
        flash("Your virtual ATM card is already frozen.", "success")
        return redirect(url_for("virtual_card_review"))

    card.status = "Frozen"
    db.session.commit()

    flash("Your virtual ATM card has been frozen.", "success")
    return redirect(url_for("virtual_card_review"))
...
def virtual_card_unfreeze():
    ...
    if card.status == "Cancelled":
        flash("A cancelled virtual ATM card cannot be unfrozen.", "error")
        return redirect(url_for("virtual_card_review"))

    if card.status == "Active":
        flash("Your virtual ATM card is already active.", "success")
        return redirect(url_for("virtual_card_review"))

    card.status = "Active"
    db.session.commit()

    flash("Your virtual ATM card has been unfrozen and is active again.", "success")
    return redirect(url_for("virtual_card_review"))
...
def virtual_card_cancel():
    ...
    if card.status == "Cancelled":
        # Keep the dashboard clean and allow a new card request.
        db.session.delete(card)
        db.session.commit()
        flash(
            "Your virtual ATM card has been cancelled. You can request a new card from your dashboard.",
            "success"
        )
        return redirect(url_for("dashboard"))

    # Cancellation permanently removes this local virtual-card record.
    # This makes the cancelled card disappear from the dashboard and
    # allows the customer to request a new card immediately.
    db.session.delete(card)
    db.session.commit()
